...
class SMARTDataset(BaseDataset):
    ...
    def process(self, internal_format):
        ...
        if center_objects is None:
            ...
            obj_trajs_future_mask, track_index_to_predict_new) = self.get_agent_data_global(
        ...
        ret_dict = {
            'scenario_id': np.array([scene_id] * len(track_index_to_predict_new)) if center_objects else np.array([scene_id]),
    ...
    def get_agent_data_global(
            self, obj_trajs_past, obj_trajs_future, track_index_to_predict, sdc_track_index, timestamps,
            obj_types
    ):
        ...
        object_time_embedding = np.zeros((num_objects, num_timestamps, num_timestamps + 1))
        ...
        obj_trajs_future_state = obj_trajs_future[:, :, [0, 1, 3, 4, 5, 6, 7, 8]]  # (x, y, vx, vy)
        ...
        track_index_to_predict = np.where(np.arange(num_objs)[valid_past_mask] == sdc_track_index)[0][0]

        return (obj_trajs_data, obj_trajs_mask.astype(bool), obj_trajs_pos, obj_trajs_last_pos,
                obj_trajs_future_state, obj_trajs_future_mask, track_index_to_predict)

    def get_map_data_global(self, map_infos, light_infos=None):
        map_polylines = np.expand_dims(map_infos['all_polylines'].copy(), axis=0).repeat(1, axis=0)
            ...
            if key != ('map_point', 'to', 'map_polygon'):
                for sub_key in sample[key].keys():
                    if sub_key == 'num_nodes':
                        ...
                        merged_data[key][sub_key] = [data[0][key][sub_key] for data in data_list]
                    try:
